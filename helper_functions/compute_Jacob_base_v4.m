function out = compute_Jacob_base_v2(param,grid,SS_stats,idx,StateSS,ControlSS)
...
switch(param.regime)
	case('Taylor')
		F23_aux(R_cb_ind,end-grid.os+R_star_ind) = (-param.rho_R*(log(R_star_aux/param.R_cb)-log(R_star_ss/param.R_cb)))./perturb_size;
	...
	case('Partial QE')
		F24_aux(R_cb_ind,end-grid.oc+pi_ind)    = - (log(pi_aux/param.pi_bar)-log(pi_ss/param.pi_bar))/perturb_size;
end
...
switch(param.regime)
	case('Taylor')
		F23_aux(A_g_ind,end-grid.os+A_g_ind)      = -( log( ((param.rho_A_g)*(A_g_aux) + (1-param.rho_A_g)*SS_stats.A_g) + eps_QE_ss*Y_ss ) - log( ((param.rho_A_g)*(A_g_ss) + (1-param.rho_A_g)*SS_stats.A_g))  )/perturb_size;
	...
	case('QE')
		...
		F21_aux(A_g_ind,end-grid.os+x_cb_ind)   = - (log( x_cb_aux*Q_ss *(A_g_ss) /Q_ss + 1 )-log( x_cb_ss*Q_ss*(A_g_ss)/Q_ss + 1 ))/perturb_size;
        
	case('Partial QE')
		F21_aux(A_g_ind,end-grid.os+x_cb_ind)   = - (log( x_cb_aux*Q_ss *(A_g_ss) /Q_ss + 1 )-log( x_cb_ss*Q_ss*(A_g_ss)/Q_ss + 1 ))/perturb_size;
		F23_aux(A_g_ind,end-grid.os+A_g_ind)    = - (log( x_cb_ss *Q_ss *(A_g_aux)/Q_ss + 1 )-log( x_cb_ss*Q_ss*(A_g_ss)/Q_ss + 1 ))/perturb_size;
end		
...
F21_aux(eta_ind,end-grid.os+p_mark_ind)        =  -( log(p_mark_aux/(p_mark_aux-1)) - log(p_mark_ss/(p_mark_ss-1)) )/perturb_size; %was negative
...
% F21_aux(eps_varphi_ind  ,end-grid.os+eps_varphi_ind)     = 1;

% 2) Control variables

% (1) K_t

% log(K) = log(A_hh + A_g  + A_b + SS_stats.A_F)
...
% F43_aux(B_ind,end-grid.os+B_F_ind)  = -((1-1/B_F_aux)-(1-1/B_F_ss))*SS_stats.Y/perturb_size;
...
% F44_aux(r_a_ind,end-grid.oc+tau_a_ind)        = - ((1-tau_a_aux)*(1-param.Eratio-param.b_share)*Profit_ss/K_ss - (1-tau_a_ss)*(1-param.Eratio-param.b_share)*Profit_ss/K_ss )/perturb_size;
