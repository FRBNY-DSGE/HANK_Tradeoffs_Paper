...
spec_settings.simul_disagg = false;
spec_settings.use_TFP_or_MK = "MK";

% Kappa Settings (No need to change)
spec_settings.high_kappa = false;
spec_settings.high_kappa_val = 0.1;

% Load in file with saved shock series
if spec_settings.use_TFP_or_MK == "TFP"
    load('shock_series_combined_hk_5_TFP_only.mat')
    % Shock label: Shock identifier in simulation results file
    spec_settings.shock_label = "shock_series_combined_hk_5_TFP_only";
    spec_settings.shock_series = tfp_series(1:end,:)';

elseif spec_settings.use_TFP_or_MK == "MK"
    load('shock_series_combined_hk_5_MK_only.mat')
    % Shock label: Shock identifier in simulation results file
    spec_settings.shock_label = "shock_series_combined_hk_5_MK_only";
    spec_settings.shock_series = mkp_series(1:end,:)';
end

spec_settings.num_simul_period = size(spec_settings.shock_series,1);

% Parameter estimation string
spec_settings.param_input = 'precovid_ugap_v4';
...
parfor phi_pi_ind = 1:length(phi_pi_vals)
    run_simul(spec_settings, phi_pi_vals, phi_pi_ind);
end
...
function run_simul(spec_settings, phi_pi_vals, phi_pi_ind)
